...
total_epoch = 20                       # number of train epochs
...
class CustomDataset(Dataset):                                   # define custom dataset (x: r, vc, los, daz, dlos), (y: hcmd)
    def __init__(self, path):
        ...
        self.len = xy.shape[0]
        self.x_data = torch.tensor(xy[:, 0:6])
        xy = xy.astype('int_')
        self.y_data = torch.tensor(xy[:, 6])
...
def train_model(num_layers, _nodes, lr, batch_size, train_loader, val_loader, model_char, system_logger):       # Function for train model
    ...
    system_logger.info(summary(model, (1, 6)))
...
def test_model(cmd_char, choose_ht, model, model_char, system_logger):
    ...
    while True:
        ...
        Vm = np.random.randint(2, 50)                    # initial speed
        Vt = np.random.randint(2, 50)
        ...
        ################# Initialize environment #################
                                                                           # initial velocity
        approach_angle = 180*np.pi/180*(2*np.random.rand()-1)                        # initial approach angle
        ...
        for k in range(N-1):
            ...
            if k > 3 and r > dist_sep and abs(elev) < 40*np.pi/180 and abs(azim) < 40*np.pi/180:
                insight += 1
                data = torch.tensor(((np.array([r, vc, azim, los, daz, dlos])
                                      - mean)/std).astype(np.float32)).cuda()
                output = model(data.view(-1, 6))
    ...
    system_logger.info("error with test " + cmd_char + " sim {}: {}".format(total_sim, err))
